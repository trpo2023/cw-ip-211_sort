#define CTEST_MA #include <algorithm>
...
inline void sortVector(std::vector<std::string> &vec,
                       bool (*sortPredicate)(const std::string &,
...
inline std::string removeLeadingNonPrintableChars(const std::string &str) {
  ...
  cleanStr.erase(cleanStr.begin(),
                 std::find_if(cleanStr.begin(), cleanStr.end(),
...
// -b
inline bool ignoreLeadingSpaces(const std::string &str1,
...
// -d
inline bool dictionaryOrderCompare(const std::string &str1,
                                   const std::string &str2) {
  size_t i = 0;
  size_t j = 0;
...
// -f
inline bool caseInsensitiveCompare(const std::string &str1,
                                   const std::string &str2) {
...
// -g
inline bool ignorePunctuationAndSpaces(const std::string &str1,
                                       const std::string &str2) {
  std::locale loc;
...
inline bool humanNumericCompare(const std::string &str1,
                                const std::string &str2) {
...
inline void displayHelp() {
  ...
  std::cout << "  -g                Ignore punctuation and spaces when sorting"
            << std::endl;
...
inline void kSortVector(std::vector<std::string> &lines,
                        std::vector<std::string> &flags) {
  auto it =
      std::find_if(flags.begin(), flags.end(), [](const std::string &flag) {
...
// -M
inline bool monthOrderCompare(const std::string &str1,
                              const std::string &str2) {
  const std::vector<std::string> months = {"JAN", "FEB", "MAR", "APR",
                                           "MAY", "JUN", "JUL", "AUG",
                                           "SEP", "OCT", "NOV", "DEC"};

  std::string upperStr1 = str1;
  std::transform(upperStr1.begin(), upperStr1.end(), upperStr1.begin(),
                 ::toupper);
  std::string upperStr2 = str2;
  std::transform(upperStr2.begin(), upperStr2.end(), upperStr2.begin(),
                 ::toupper);
...
inline bool numericOrderCompare(const std::string &str1,
                                const std::string &str2) {
...
inline void randomSort(std::vector<std::string> &lines,
  ...
  if (itf != flags.end()) {
    std::string file =
        itf->substr(16); // Получаем имя файла после "--random-source="
    randomSort(lines,
...
// -V
inline bool versionCompare(const std::string &version1,
